...
from utils.get_hash import get_hash1
...
class PassportManager(models.Manager):
	def add_one_passport(self, username, password, email):
		'''添加一个账户信息'''
		passport = self.create(username=username,password=get_hash1(password),email=email)
		#返回passport
		return passport
	...
	def get_one_passport(self,username, password):
		'''根据用户密码查找账户信息'''
		try:
			passport = self.get(username=username,password=get_hash1(password))
			return passport
